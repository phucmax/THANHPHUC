...
-- AUTO STEAL REWORKED
-- ------------------------------------------------------------
-- No filters / method menus.
-- Target source: workspace.AreaEggSlotsClient.
-- Target priority: highest rarity -> highest KG/weight -> largest size.
-- Flow: fly to target -> confirm arrival -> release movement ->
--       fire zero-duration E once -> wait until the egg is actually
--       in the player's hand -> pull back to base -> repeat.
-- ============================================================

local function AutoStealGetNumber(value)
    ...
    if type(value) == "string" then
        ...
        local amount = normalized:match("([%+%-]?%d+%.?%d*)")
...
local function AutoStealGetAttribute(instance, keys)
...
local function AutoStealReadNumber(instance, keys)
    local direct = AutoStealGetAttribute(instance, keys)
    local directNumber = AutoStealGetNumber(direct)

    if directNumber > 0 then
        return directNumber
    end

    for _, descendant in ipairs(instance:GetDescendants()) do
        local value = AutoStealGetAttribute(descendant, keys)
        local number = AutoStealGetNumber(value)
        ...
        if descendant:IsA("NumberValue") or descendant:IsA("IntValue") then
            for _, key in ipairs(keys) do
                if descendant.Name:lower() == tostring(key):lower() then
        ...
        elseif descendant:IsA("StringValue") then
            for _, key in ipairs(keys) do
                if descendant.Name:lower() == tostring(key):lower() then
                    return AutoStealGetNumber(descendant.Value)
                end
            end
        end
    end
...
local function AutoStealReadText(instance, keys)
    local value = AutoStealGetAttribute(instance, keys)
    if value ~= nil then
        return tostring(value)
    end

    for _, descendant in ipairs(instance:GetDescendants()) do
        local v = AutoStealGetAttribute(descendant, keys)
        if v ~= nil then
            return tostring(v)
        end
...
local function AutoStealRarityRank(name)
    local rarity = tostring(name or ""):lower()
    ...
    for key, rank in pairs(aliases) do
        if rarity:find(key, 1, true) then
            return rank
        end
    end

    return iData.value10.rarityLadder[name] or 0
end

local function AutoStealGetRarity(instance)
    local value = AutoStealReadText(instance, {
        "Rarity",
        "RarityName",
        "RarityId",
        "Tier",
    ...
    if value then
        return value, AutoStealRarityRank(value)
    end

    local lowerName = instance.Name:lower()
    local bestName, bestRank = nil, 0

    for _, rarity in ipairs(iData.value10.Rarities) do
        local rank = AutoStealRarityRank(rarity)
        if lowerName:find(rarity:lower(), 1, true) and rank > bestRank then
            bestName, bestRank = rarity, rank
        end
    end

    return bestName or "Unknown", bestRank
end

local function AutoStealGetWeight(instance)
    return AutoStealReadNumber(instance, {
        "WeightKg",
        "Weight",
        "KG",
        "Kg",
...
local function AutoStealGetSize(instance)
    ...
    return part and part.Size.Magnitude or 0
end

local function AutoStealGetPosition(instance)
...
local function AutoStealGetUid(instance)
    local uid = AutoStealGetAttribute(instance, {"UID", "Uid", "uid", "EggUID", "EggUid"})
    if uid ~= nil then
        return tostring(uid)
    end

    local modelUid = instance.Name:match("_(%w+)$")
    return modelUid or instance.Name
end

local function AutoStealFindBestWorkspaceEgg()
    ...
    for _, egg in ipairs(folder:GetChildren()) do
        if not (egg:IsA("Model") or egg:IsA("Folder") or egg:IsA("BasePart")) then
            continue
        end

        local position = AutoStealGetPosition(egg)
        if not position then
            continue
        end

        local rarityName, rarityRank = AutoStealGetRarity(egg)
        local weight = AutoStealGetWeight(egg)
        local size = AutoStealGetSize(egg)
        local uid = AutoStealGetUid(egg)
        ...
        if not best then
            better = true
        elseif candidate.rarityRank ~= best.rarityRank then
            better = candidate.rarityRank > best.rarityRank
        elseif candidate.weight ~= best.weight then
            better = candidate.weight > best.weight
        elseif candidate.size ~= best.size then
            better = candidate.size > best.size
        else
            local root = iData.value21()
            if root then
                local a = (candidate.pos - root.Position).Magnitude
                local b = (best.pos - root.Position).Magnitude
                better = a < b
            end
        end
...
local function AutoStealFindPrompt(candidate)
    if not candidate or not candidate.instance then
        return nil
    end

    local fallback

    for _, descendant in ipairs(candidate.instance:GetDescendants()) do
        if descendant:IsA("ProximityPrompt") and descendant.Enabled then
            local text = (
                tostring(descendant.Name) .. " "
                .. tostring(descendant.ActionText) .. " "
...
local function AutoStealHoldingTarget(candidate)
    ...
    local expectedUid = tostring(candidate.uid or "")

    for _, child in ipairs(character:GetChildren()) do
        if child:IsA("Tool") then
            local uid = AutoStealGetAttribute(child, {"UID", "Uid", "uid", "EggUID", "EggUid"})
            if uid ~= nil and tostring(uid) == expectedUid then
                return true
            end

            for _, descendant in ipairs(child:GetDescendants()) do
                if descendant:IsA("Model") then
                    if descendant.Name:find(expectedUid, 1, true) then
                        return true
                    end
                end
            end
...
local function AutoStealActivatePrompt(candidate)
    local prompt = AutoStealFindPrompt(candidate)
    if not prompt then
        return false
    end
    ...
    task.delay(0.15, function()
        pcall(function()
            if prompt.Parent and oldHold ~= nil then
...
local function AutoStealReturnToBase()
    ...
    if not base then
        iData.value18("Auto Steal", "Không tìm thấy SpawnLocation để kéo về.", 5)
        return false
    end
    ...
    -- First jump exactly 100 studs away from SpawnLocation, on the same
    -- line from SpawnLocation -> current player position. This is ONE
    -- teleport only; the remaining trip uses the configured smooth speed.
    local spawnPos = base
    local flat = Vector3.new(
        root.Position.X - spawnPos.X,
        0,
        root.Position.Z - spawnPos.Z
    )

    local distance = flat.Magnitude
    if distance > 100 then
        local safePos = Vector3.new(
            spawnPos.X + flat.Unit.X * 100,
            root.Position.Y,
            spawnPos.Z + flat.Unit.Z * 100
        )
    ...
    -- Smooth final run. No repeated CFrame teleporting.
    local returned = false
    local ok = pcall(function()
        returned = updateGoToTreadmill(base, 8, function()
            return iData.value14.AutoSteal
        end, false)
    end)

    return ok and returned
end
local autoStealTarget = nil
local autoStealWaitStarted = 0
local autoStealPromptSent = false
local autoStealTargetLost = false
local autoStealRemoteSignal = 0
local autoStealRemoteUid = nil

-- Observe the exact RF/EggWorld/AskFieldEggCarry call. This is only used
-- by Auto Steal as the pickup confirmation trigger.
do
    local targetRemoteName = "RF/EggWorld/AskFieldEggCarry"
    pcall(function()
        if hookmetamethod and getnamecallmethod then
            local oldNamecall
            oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
                local method = getnamecallmethod()
                if self and self.Name == targetRemoteName and method == "InvokeServer" then
                    local args = {...}
                    local payload = args[1]
                    if type(payload) == "table" then
                        autoStealRemoteUid = payload.Uid and tostring(payload.Uid) or nil
                    else
                        autoStealRemoteUid = nil
                    end
                    autoStealRemoteSignal = os.clock()
                end
...
iData.value11.AutoStealPhase = "Idle"

iData.value11.AutoStealReached = false

iData.value11.AutoStealNotice = false

handler(function()
    while iData.value17() do
        if not iData.value14.AutoSteal then
            autoStealTarget = nil
            autoStealPromptSent = false
            autoStealTargetLost = false
            iData.value11.AutoStealTarget = nil
            iData.value11.AutoStealPhase = "Idle"
            iData.value11.AutoStealReached = false
            task.wait(0.2)
        ...
        local success, err = xpcall(function()
            -- Phase 1: choose the best currently visible egg directly from
            -- workspace.AreaEggSlotsClient. No filters are consulted.
            if not autoStealTarget then
                autoStealTarget = AutoStealFindBestWorkspaceEgg()
                ...
                autoStealPromptSent = false
                autoStealTargetLost = false
                iData.value11.AutoStealTarget = autoStealTarget
                iData.value11.AutoStealPhase = "Flying"
                iData.value11.AutoStealReached = false
                iData.value11.AutoStealNotice = false
            end

            if not autoStealTarget.instance or not autoStealTarget.instance.Parent then
                -- If the selected egg vanished, rescan immediately.
                autoStealTarget = nil
                iData.value11.AutoStealTarget = nil
                iData.value11.AutoStealPhase = "Rescanning"
                task.wait(0.08)
                return
            end

            local livePosition = AutoStealGetPosition(autoStealTarget.instance)
            if livePosition then
                autoStealTarget.pos = livePosition
            end

            -- Phase 2: travel to target. Run Mode uses Humanoid movement;
            -- normal mode keeps the legacy travel engine intact.
            if not iData.value11.AutoStealReached then
                local reached = false

                if iData.value14.AutoStealRunMode then
                    local root = iData.value21()
                    local humanoid = iData.value22()
                    local distance = root and (Vector3.new(
                        autoStealTarget.pos.X, 0, autoStealTarget.pos.Z
                    ) - Vector3.new(
                        root.Position.X, 0, root.Position.Z
                    )).Magnitude or math.huge

                    if humanoid and distance > 8 then
                        pcall(function()
                            humanoid.WalkSpeed = math.clamp(
                                tonumber(iData.value119.SpeedValue) or 100,
                                1, 1000
                            )
                            humanoid:MoveTo(autoStealTarget.pos)
                        end)
                        iData.value11.travelling = true
                        iData.value11.travelStep = tick()
                        task.wait(0.03)
                    ...
                    iData.value10.TRAVEL_SPEED = math.clamp(
                        tonumber(iData.value14.AutoStealSpeed) or 1000,
                        1, 1000
                    )
                    reached = updateGoToTreadmill(
                        autoStealTarget.pos,
                        8,
                        function()
                            return iData.value14.AutoSteal
                                and autoStealTarget
                                and autoStealTarget.instance
                                and autoStealTarget.instance.Parent ~= nil
                        end,
                        true
                    )
                end
                ...
                if reached or distance <= 8 then
                    iData.value11.AutoStealReached = true
                    iData.value11.AutoStealPhase = "Reached"
                    iData.value11.AutoStealNotice = true

                    iData.value11.travelling = false
                    iData.value11.travelToken = iData.value11.travelToken + 1
                    pcall(iData.value51)

                    -- Briefly hold the character at the egg, then release it.
                    pcall(function()
                        local r = iData.value21()
                        if r then
                            r.CFrame = CFrame.new(autoStealTarget.pos + Vector3.new(0, 0.5, 0))
                    ...
                    -- The player is free again before pickup verification.
                    iData.value11.travelling = false
                end
            ...
            -- Phase 3: stay completely still at the egg.
            -- We do NOT run Auto Pick here. We only wait for the actual
            -- AskFieldEggCarry RemoteFunction call to appear.
            if not autoStealPromptSent then
                autoStealPromptSent = true
                iData.value11.AutoStealPhase = "WaitingForRF"
                autoStealRemoteSignal = 0
                autoStealRemoteUid = nil
                AutoStealActivatePrompt(autoStealTarget)
                autoStealWaitStarted = tick()
                iData.value11.travelling = false
                iData.value11.travelToken = iData.value11.travelToken + 1
                pcall(iData.value51)
            end

            -- Do absolutely nothing to movement while waiting. The instant
            -- the exact RF is observed, return to SpawnLocation.
            if autoStealRemoteSignal > 0
                and tick() - autoStealRemoteSignal < 1.5 then

                iData.value11.AutoStealPhase = "RFDetected"
                iData.value18("Auto Steal", "AskFieldEggCarry detected • returning.", 2)

                iData.value11.travelling = false
                iData.value11.travelToken = iData.value11.travelToken + 1
                pcall(iData.value51)

                AutoStealReturnToBase()
            ...
            -- If the egg has disappeared from the slot after the player
            -- interacted with it, keep a short observation window before
            -- deciding that the attempt failed.
            if not autoStealTarget.instance.Parent then
                if tick() - autoStealWaitStarted <= 2.5 then
                    task.wait(0.05)
                    return
                end

                autoStealTargetLost = true
            end

            if tick() - autoStealWaitStarted > 10 then
                iData.value18("Auto Steal", "Không thấy AskFieldEggCarry • đổi mục tiêu.", 3)
                autoStealTarget = nil
                autoStealPromptSent = false
                autoStealTargetLost = false
                iData.value11.AutoStealTarget = nil
        ...
        if not success then
            warn("[PHUCMAX][AutoSteal] " .. tostring(err))
            iData.value18("Auto Steal", "Lỗi vòng Auto Steal — đang thử lại.", 4)
            autoStealTarget = nil
            iData.value11.AutoStealTarget = nil
            iData.value11.AutoStealPhase = "ErrorRecovery"
            task.wait(0.35)
        end
    end
end)
...
-- Auto Steal + Auto Steal Free are logical feature groups inside MAIN.
-- ============================================================
...
-- MAIN = Auto Steal + Auto Steal Free (merged physical tab)
local SHARD_VI = {
...
local function SHARD_FindTab(title)
    -- Auto Steal + Auto Steal Free share one physical MAIN tab.
    title = tostring(title or "")

    if title == "Auto Steal" or title == "Auto Steal Free" then
        return "MAIN"
...
function iData.value133(vector, number)
	...
	while iData.value17() and iData.value14.AutoSteal and not (sum < tick()) do
		local secondaryInput = iData.value21()
		local flag = iData.value22()
...
handler(function()
	while iData.value17() do
		...
		if iData.value14.AutoSteal and iData.value23() and (iData.value132() and not iData.value126) then
			iData.value126 = true
...
function iData.value138()
	...
	parent.MouseButton1Click:Connect(function()
		iData.value137(not iData.value14.AutoSteal)
	end)
end
...
iData.value12.TabSteal:Toggle({
	...
	Desc = "Bật đúng cơ chế Bypass Humanoid cũ.",
	Value = false,
	Callback = function(v)
		iData.value14.AntiCheat = v
		iData.value119.HumReady = false
		if v then
			task.spawn(function()
				local ready=createAutoStealHumanoid()
				iData.value119.HumReady=ready
				iData.value14.HumReady=ready
				iData.value18("Anti-Cheat",not ready and "Not ready." or "Bypass active.")
			end)
		else
...
iData.value12.TabSteal:Slider({
	Title = "Auto Steal Speed",
	Desc = "Tốc độ Auto Steal • ",
	Step = 1,
	Value = {Min=1,Max=1000,Default=math.clamp(iData.value14.AutoStealSpeed or 100,1,1000)},
	Callback = function(v)
		iData.value14.AutoStealSpeed=math.clamp(tonumber(v) or 100,1,1000)
		iData.value10.TRAVEL_SPEED=iData.value14.AutoStealSpeed
	end,
})
iData.value12.TabSteal:Toggle({
	Title = "Run Mode",
	Desc = "Chạy bằng WalkSpeed thay vì travel engine.",
	Value = false,
	Callback = function(v) iData.value14.AutoStealRunMode=v end,
})
iData.value12.TabSteal:Slider({
	Title = "Run Speed",
	Desc = "Tốc độ chạy • tối đa 2000.",
	Step = 1,
	Value = {Min=1,Max=2000,Default=100},
	Callback = function(v) iData.value119.SpeedValue=math.clamp(tonumber(v) or 100,1,2000) end,
})
iData.value12.TabSteal:Toggle({
	Title = "Mini Spawn UI",
	Desc = "Hiện UI nhỏ chỉ có nút chạy về spawn.",
	Value = false,
	Callback = function(v)
		local gui=iData.value11.spawnMiniGui
		if gui and gui.Parent then gui.Enabled=v end
	end,
})

iData.value41.AutoSteal = iData.value12.TabSteal:Toggle({
    Title = "Auto Steal",
    Desc = "Tự động bay tới trứng tốt nhất, thả tự do, chờ trứng vào tay rồi kéo về.",
    Value = false,
    Callback = function(autoSteal)
        ...
        if autoSteal then
            ...
            iData.value11.AutoStealReached = false
            iData.value11.AutoStealNotice = false
            iData.value18("Auto Steal", "Đang quét trứng tốt nhất...", 3)
        else
            iData.value11.AutoStealTarget = nil
            ...
            iData.value11.travelToken = iData.value11.travelToken + 1
            iData.value51()
            iData.value18("Auto Steal", "Đã dừng.", 3)
        end
    end,
})

-- Auto Steal has no selectable menus: its behavior is fully automatic.
...
iData.value12.TabHop:Button({
	...
	Desc = "Ưu tiên server đang có đúng 1 người.",
	Callback = function()
		task.spawn(function()
...
iData.value12.TabHop:Button({
	...
	Callback = function()
		task.spawn(function()
			...
			iData.value18("Recall", not (if vector then updateGoToTreadmill(vector,8) else false) and "No spawn point found." or "Back at spawn.")
		end)
	end,
})
...
iData.value12.TabSell:Toggle({
	Title = "Speed Changer",
	Desc = "Khóa WalkSpeed bằng cơ chế cũ • cần Bypass.",
	Value = false,
	Callback = function(v)
...
iData.value12.TabSell:Slider({
	Title = "Walk Speed",
	Desc = "Tốc độ Player • tối đa 2000.",
	Step = 1,
	Value = {Min = 16, Max = 2000, Default = math.clamp(iData.value119.SpeedValue or 60, 16, 2000)},
	Callback = function(v)
		iData.value119.SpeedValue = math.clamp(tonumber(v) or 60, 16, 2000)
		if iData.value119.SpeedChanger and iData.value119.HumReady then
			local hum = iData.value22()
			if hum then pcall(function() hum.WalkSpeed = iData.value119.SpeedValue end) end
		end
	end,
})
...
-- SETTINGS / CONFIGURATION UI REMOVED BY DESIGN.
-- Persistent configuration is now internal only; no visible controls.
iData.value142 = {
	spawned = {},
	orbitRadius = 4,
	orbitSpeed = 0.8,
...
if not __PHUCMAX_FEATURE_OK then
    ...
    pcall(function()
        if type(_G.PHUCMAX_Notify) == "function" then
            _G.PHUCMAX_Notify("PHUCMAX • UI + chức năng đã tải", 3)
        end
    end)
end

-- ============================================================
-- AUTO STEAL STANDALONE UI
-- ============================================================
-- Compact draggable controller for the Auto Steal state.
-- Uses the existing iData.value14.AutoSteal flag and reports the
-- current target/phase without depending on the main window library.
-- ============================================================
do
    local Players = game:GetService("Players")
    ...
    local oldGui = guiParent:FindFirstChild("PHUCMAX_AutoSteal_Controller")
    if oldGui then
        oldGui:Destroy()
    end

    local gui = Instance.new("ScreenGui")
    gui.Name = "PHUCMAX_AutoSteal_Controller"
    ...
    shadow.Position = UDim2.new(1, -24, 0, 120)
    shadow.Size = UDim2.fromOffset(282, 154)
    ...
    panel.Position = UDim2.new(1, -30, 0, 114)
    panel.Size = UDim2.fromOffset(282, 154)
    ...
    stroke.Color = Color3.fromRGB(75, 145, 235)
    ...
    title.Size = UDim2.fromOffset(190, 28)
    ...
    local sub = Instance.new("TextLabel")
    sub.BackgroundTransparency = 1
    sub.Position = UDim2.fromOffset(17, 35)
    sub.Size = UDim2.fromOffset(205, 20)
    sub.Font = Enum.Font.Gotham
    sub.Text = "Target scanner • live controller"
    sub.TextSize = 10
    sub.TextColor3 = Color3.fromRGB(145, 151, 166)
    sub.TextXAlignment = Enum.TextXAlignment.Left
    sub.ZIndex = 3
    sub.Parent = panel
    ...
    toggle.Position = UDim2.new(1, -14, 0, 12)
    ...
    status.Position = UDim2.fromOffset(17, 61)
    status.Size = UDim2.fromOffset(248, 20)
    ...
    target.Position = UDim2.fromOffset(17, 86)
    target.Size = UDim2.fromOffset(248, 22)
    target.Font = Enum.Font.Gotham
    target.TextSize = 10
    target.TextColor3 = Color3.fromRGB(184, 189, 202)
    ...
    phase.Position = UDim2.fromOffset(17, 111)
    phase.Size = UDim2.fromOffset(248, 22)
    phase.Font = Enum.Font.Gotham
    phase.TextSize = 10
    phase.TextColor3 = Color3.fromRGB(135, 143, 160)
    ...
    dragHandle.Size = UDim2.new(1, 0, 0, 55)
    dragHandle.ZIndex = 5
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        panel.Position = nextPos
        shadow.Position = UDim2.new(
            nextPos.X.Scale, nextPos.X.Offset + 6,
            nextPos.Y.Scale, nextPos.Y.Offset + 6
        )
    end)

    local function setVisual(enabled)
        toggle.Text = enabled and "ON" or "OFF"
        toggle.BackgroundColor3 = enabled
            and Color3.fromRGB(48, 178, 112)
            or Color3.fromRGB(65, 69, 82)
        status.Text = enabled and "● ACTIVE" or "● STANDBY"
        status.TextColor3 = enabled
            and Color3.fromRGB(88, 221, 145)
            or Color3.fromRGB(151, 157, 171)
    end

    toggle.MouseButton1Click:Connect(function()
        iData.value14.AutoSteal = not iData.value14.AutoSteal
        setVisual(iData.value14.AutoSteal)
    end)

    task.spawn(function()
        while gui.Parent do
            local enabled = iData.value14.AutoSteal
            setVisual(enabled)

            local candidate = iData.value11.AutoStealTarget
            if candidate and candidate.instance then
                local rarity = tostring(candidate.rarity or "Unknown")
                local kg = tonumber(candidate.weight) or 0
                target.Text = string.format("Target: %s  •  %.2f KG", rarity, kg)
            else
                target.Text = "Target: scanning..."
            end

            phase.Text = "Phase: " .. tostring(iData.value11.AutoStealPhase or "Idle")
            task.wait(0.12)
        end
    end)
end
